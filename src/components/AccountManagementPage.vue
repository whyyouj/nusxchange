...
<v-dialog v-model="showDeleteModal" max-width="700" style= "  margin: auto; display: flex; flex-direction: column;justify-content: center;height: 100%;">
  ...
  <div class="photo">
    ...
    <button @click="showPhotoModal = true" style="  background-color: rgb(204, 204, 204, 0.5);
  padding: 5px 10px;
  border-radius: 10px;
  margin-left: 2%;
...
<div class="info" style="margin-top: 5%">
  ...
  padding: 5px 10px;
  border-radius: 10px;
  font-size: 15px; font-weight: bold">Change</button>
      ...
      async changePassword(newPw) {
        ...
        this.refreshComp += 1;
            ...
            if (error.code === "auth/requires-recent-login") {
              ...
              console.log("User needs to reauthenticate before updating their password.");
            } else {
    ...
    updateTeleList() {
      if (this.tele !== "-" && this.exchangeuni !== "None") {
        this.teleList = this.teleListAll.filter(item => {
          return item.exchangeUniversity === this.exchangeuni && item.semester === this.semester && this.email !== item.email
        })
      } else {
        this.teleList = []
      }
